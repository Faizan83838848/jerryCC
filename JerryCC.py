...
def logo():
    ...
    return (gn + figlet)
print (logo())
print (bl + "[-] Powered by Jerry ( Faizan Khan ) ")
print (gn + "[+] Made By FAIZAN")
print (cy + "[=] Jerry CC Tools Version : 3.7")
...
def genscard():
    ...
    return (gn + "[-] Brand : %s\n[-] Card Number : %s\n[-] Bank : %s\n[-] Name : %s\n[-] Address : %s\n[-] Country : %s\n[-] Money Range : %s\n[-] CVV : %s\n[-] Expiry : %s\n[-] Pin : %s\n============================\n[*] Telegram Channel : @sedxjerryhack" % (card['IssuingNetwork'] , card['CardNumber'] , card['Bank'] , card['Name'] , card['Address'] , card['Country'] , card['MoneyRange'] , card['CVV'] , card['Expiry'] , card['Pin']) + cv)
...
def genmcard():
    ...
    rs = requests.post(sitex , headers=headers , cookies=cookies,data=payload)
    data = json.loads(rs.text)
    open("JerryCC_card.txt","w").write("")
    for i in range(1,10):
        card = data['creditCard'][i]
        f = open("JerryCC_card.txt","a")
...
if opr == "1":
    ...
    print (mag + "[$] Telegram Channel : @sedxjerryhack" + cv)
